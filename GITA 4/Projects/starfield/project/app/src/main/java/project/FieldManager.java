...
import project.Starmaker.Shape;
...
public class FieldManager extends JFrame implements ActionListener{
    ...
    //declare components
    private final Starmaker starmaker = new Starmaker(10, 10);
    ...
    public void action(){
        outputArea.setText(starmaker.createShape(Shape.DOWN_RIGHT));

        outputArea.append(Constants.LINEBREAK);

        outputArea.append(starmaker.createShape(Shape.UP_RIGHT));

        outputArea.append(Constants.LINEBREAK);

        outputArea.append(starmaker.createShape(Shape.UP_LEFT));
    }
